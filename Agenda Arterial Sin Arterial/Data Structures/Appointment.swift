...
class Appointment: Identifiable {
    ...
    func uploadAppointment() -> (Bool, String, String){
        ...
        if appIsValid {
            ...
            let info : [String: Any] = ["id": idC, "idPaciente": idUser,"fecha": self.date, "idMedico": self.idMedic,  "comentario": self.comments]

            login.saveData(collectionName: "consultas", id: idC, info: info){(done)
                in
                if done{
                    ...
                    for n in 1...5 {
                        print("Here2")
                        let idM = UUID().uuidString
                        let nextMeditionDate = Calendar.current.date(byAdding: DateComponents(day: -n), to:self.date)!
                        print("Here2.1 \(nextMeditionDate)")
                        if nextMeditionDate > Date(){
                            print("Here2.2")
                            let infoM : [String: Any] = ["id": idM, "idPaciente": idUser, "fecha":nextMeditionDate, "tipo":"medicion","titulo":"Próxima Medición", "descripcion":"Ve a Tu Presion y captura tu presion"]
                            print(infoM)
                            self.login.saveData(collectionName: "notificaciones", id: idM, info: infoM){
                                (done)in
                                if done{
                    ...
                    self.login.saveData(collectionName: "notificaciones", id: id1, info: info1){
                        (done)in
                        if done{
                            print("Succesfully upload reminder of type semanal")
                        }else{
                            print("ERROR saving info")
                        }
                    }
                    ...
                    self.login.saveData(collectionName: "notificaciones", id: id2, info: info2){
                        (done)in
                        if done{
                            print("Succesfully upload reminder of type salud")
                        }else{
                            print("ERROR saving info")
                        }
                    }
                    ...
                    let info3 : [String: Any] = ["id": id3, "idPaciente": idUser, "fecha":self.date, "tipo":"consulta","titulo":"Próxima Consulta", "descripcion":self.comments, "idConsulta":idC]
                    self.login.saveData(collectionName: "notificaciones", id: id3, info: info3){
                        (done)in
                        if done{
                            print("Succesfully upload reminder of type consulta")
                        }else{
                            print("ERROR saving info")
                        }
                    }
