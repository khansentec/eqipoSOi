...
class Medition : Identifiable{
    ...
    func validateCapture(presionSupStr : String? , presionInfStr : String?, pulse: Int) -> (Bool, String){
        if presionSupStr != "" || presionInfStr != "" {
            if let presionSup = Int(presionSupStr!),
               let presionInf = Int(presionInfStr!){
                // presion valida sin pulso
                print(presionInf)
                print(presionSup)
<<<<<<< HEAD:Agenda Arterial Sin Arterial/Views/Medition.swift
                if (pulso >= 1) && (pulso <= 999){
                    // presion valida con pulso
                    print(pulso)
                    return (true, "Captura válida con pulso")
=======
...
>>>>>>> ofront:Agenda Arterial Sin Arterial/Data Structures/Medition.swift
                }else{
